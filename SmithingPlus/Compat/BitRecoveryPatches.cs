...
{
    [HarmonyPostfix]
    [HarmonyPatch(nameof(ItemNugget.OnCreatedByCrafting)), HarmonyPriority(Priority.Last)]
        ...
        GridRecipe byRecipe)
    {
        ...
        var world = api.World;
        var smithingRecipe = allInputslots.FirstOrDefault(slot => slot.Itemstack?.GetSmithingRecipe(world) != null)?.Itemstack?.GetSmithingRecipe(world);
